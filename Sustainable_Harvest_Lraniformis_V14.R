...
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
# Graphing output.csv from Sustainable_Harvest_Lraniformis_V14
# plot finishing is completed in another program
# R Mathwin October 2023
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
## Remove everything
rm(list = ls())
...
for (st in 1:length(siteList)) {
  site <- siteList[st]
  assign(paste0(site,"startPops"),read.csv(paste0("C:/Workspace/SustHarvRevSep22/startPops",site,"2.csv")))   
  for (test in 1:1) {
    test <- "3NN2"
    for (stage in 1:3) {
      stg <- stgList[stage]
...
# Please see below for the too hard basket
colnames(Hogwash.dryExt.egg) <- colnames(NapNap.dryExt.adult) <- colnames(NapNap.dryExt.tad) <- colnames(NapNap.dryExt.egg) <- reduction
colnames(Epsom.dryExt.adult) <- colnames(Epsom.dryExt.tad) <- colnames(Epsom.dryExt.egg) <- colnames(Hogwash.dryExt.adult) <- colnames(Hogwash.dryExt.tad) <- reduction
...
for (j in 1:length(stgList)) {
  stg <- stgList[j]
  cc <- sum(get(paste0("Hogwash.dryExt.",stg))[1,]) + sum(get(paste0("NapNap.dryExt.",stg))[1,]) + sum(get(paste0("Epsom.dryExt.",stg))[1,])
  dd <- sum(get(paste0("Hogwash.dryExt.",stg))[2,]) + sum(get(paste0("NapNap.dryExt.",stg))[2,]) + sum(get(paste0("Epsom.dryExt.",stg))[2,])
  sum(cleanExt[stage,])
  stageRisk[2,j] <- ((cc/sum(cleanExt[stage,]))*100)
  stageRisk[3,j] <- ((dd/sum(cleanExt[stage,]))*100)
  stageRisk[1,j] <- 100 - (stageRisk[2,j] + stageRisk[3,j])
}
stageRisk
...
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
# plot probability of extinction for Figure 2. (image is finalised in image manipulation software)
# Epsom
plot(PrExtdf$remNum,PrExtdf$EpsomadultPrExt,type = "l",col = "red", xlab = "", ylab = "",
     xaxs = "i",yaxs = "i",axes = F,cex.axis = 1,ylim = c(0, 1),xlim = c(0, 1),lwd = 3, main = "")
...
lines(PrExtdf$remNum,PrExtdf$EpsomeggPrExt, type = "l", 
      col="chocolate4",lwd = 3.5)
lines(PrExtdf$remNum,PrExtdf$EpsomtadPrExt, type = "l", lty = 5, pch = 19, 
      col="blue",lwd = 3)
...
lines(PrExtdf$remNum,PrExtdf$NapNapeggPrExt, type = "l", 
      col="chocolate4",lwd = 3.5)
lines(PrExtdf$remNum,PrExtdf$NapNaptadPrExt, type = "l", lty = 5, pch = 19, 
      col="blue",lwd = 3)
...
lines(PrExtdf$remNum,PrExtdf$HogwasheggPrExt, type = "l", 
      col="chocolate4",lwd = 3.5)
lines(PrExtdf$remNum,PrExtdf$HogwashtadPrExt, type = "l", lty = 5, pch = 19, 
      col="blue",lwd = 3)


### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
# plot minimum adult female population for Figure 3 (image is finalised in image manipulation software)
...
# prep the adult start pops
minP.reduc.df <- data.frame(reduction, remNum)
# holder is a 4d array. [site(NapNap,Hogwash,Epsom)-2,stages-3, max,mn,min-3,reductions-21]
minN.divStart <- array(data = NA, dim = 3*3*21*3)
dim(minN.divStart) <- c(3,3,3,21)

# can be refactored to use lapply I think
for (j in 1:length(siteList)) {
  site <- siteList[j]
  for (k in 1:length(stgList)) {
    stg <- stgList[k] 
    for (i in 1:length(reduction)) {
      for (l in 1:10000) {
        holder[l] <-  get(paste0(site, ".minAdpop.", stg))[i,l]/get(paste0(site, "startadults"))[l]
      }
      holder[is.infinite(holder)]<-NA
      minN.divStart[j,k,1,i] <- quantile(holder, probs=0.975, na.rm=T)
      minN.divStart[j,k,2,i] <- mean(holder, na.rm=T)
      minN.divStart[j,k,3,i] <- quantile(holder, probs=0.025, na.rm=T)
      holder[] <- 0
    }
    aa <- minN.divStart[j,k,1,]
    minP.reduc.df <- cbind(minP.reduc.df, aa)
    colnames(minP.reduc.df)[ncol(minP.reduc.df)] <- paste0(site, stg,"up")
    
    aa <- minN.divStart[j,k,2,]
    minP.reduc.df <- cbind(minP.reduc.df, aa)
    colnames(minP.reduc.df)[ncol(minP.reduc.df)] <- paste0(site, stg,"mn")
    
    aa <- minN.divStart[j,k,3,]
    minP.reduc.df <- cbind(minP.reduc.df, aa)
    colnames(minP.reduc.df)[ncol(minP.reduc.df)] <- paste0(site, stg,"low")
  }
}

# rStoch plot (plotted separated by site to enable fine tuning)
...
HeggR <-  ggplot(data=rstoch.df, aes(x=remNum,y=Hogwash.rStoch.eggup), colour=Flow) + 
  ...
  geom_line(linetype="solid",color="chocolate4",size=1.2, aes(x=remNum, y=Hogwash.rStoch.eggmn), show.legend = TRUE) 
...
HtadR <-  ggplot(data=rstoch.df, aes(x=remNum,y=Hogwash.rStoch.tadup), colour=Flow) + 
  ...
  geom_hline(yintercept=0, linetype="solid",color="black",size=1) +
  geom_vline(xintercept=vert, linetype="longdash", color="grey55", size=1) +
  geom_line(linetype="longdash",color="blue", size=1.2, aes(x=remNum, y=Hogwash.rStoch.tadmn), show.legend = TRUE) 
...
HadultR <-  ggplot(data=rstoch.df, aes(x=remNum,y=Hogwash.rStoch.adultup), colour=Flow) + 
  ...
  geom_line(linetype="solid",color="red", size=1.2, aes(x=remNum, y=Hogwash.rStoch.adultmn), show.legend = TRUE) 
...
NeggR <-  ggplot(data=rstoch.df, aes(x=remNum,y=NapNap.rStoch.eggup), colour=Flow) + 
  ...
  geom_line(linetype="solid", color="chocolate4", size=1.2, aes(x=remNum, y=NapNap.rStoch.eggmn), show.legend = TRUE) 
...
NtadR <-  ggplot(data=rstoch.df, aes(x=remNum,y=NapNap.rStoch.tadup), colour=Flow) + 
  ...
  geom_line(linetype="longdash",color="blue", size=1.2, aes(x=remNum, y=NapNap.rStoch.tadmn), show.legend = TRUE) 
...
NadultR <-  ggplot(data=rstoch.df, aes(x=remNum,y=NapNap.rStoch.adultup), colour=Flow) + 
  ...
  geom_line(linetype="solid", color="red",size=1.2, aes(x=remNum,y=NapNap.rStoch.adultmn), show.legend = TRUE) 
...
EeggR <-  ggplot(data=rstoch.df, aes(x=remNum,y=Epsom.rStoch.eggup), colour=Flow) + 
  ...
  geom_line(linetype="solid", color="chocolate4", size=1.2, aes(x=remNum, y=Epsom.rStoch.eggmn), show.legend = TRUE) 
...
EtadR <-  ggplot(data=rstoch.df, aes(x=remNum,y=Epsom.rStoch.tadup), colour=Flow) + 
  ...
  geom_hline(yintercept=0, linetype="solid", color="black",size=1) +
  geom_vline(xintercept=vert, linetype="longdash", color="grey55",size=1) +
  geom_line(linetype="longdash", color="blue", size=1.2, aes(x=remNum, y=Epsom.rStoch.tadmn), show.legend = TRUE) 
...
EadultR <-  ggplot(data=rstoch.df, aes(x=remNum,y=Epsom.rStoch.adultup), colour=Flow) + 
  ...
  geom_line(linetype = "solid", color="red",size=1.2, aes(x=remNum, y=Epsom.rStoch.adultmn), show.legend = TRUE) 
...
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
# Plot mean r of adult population for Figure 4 (image is finalised in image manipulation software)
# smoothed ecdf plots (0,25,50 and 75% removal)
...
incCol <- c("chocolate4","blue","red")
cdfUpperList <- c(30,5,130)
cdfTickUp <- c(30,5,120)
cdfTickInc <- cdfTickLow <- c(5,1,20)
cdfLines <- c(1,5,1)
...
for (j in 1:3) {
  site <- siteList[j]
  cdfUp <- cdfUpperList[j]
  tickUp <- cdfTickUp[j]
  tickLow <- cdfTickLow[j]
  tickInc <- cdfTickInc[j]
  for (k in 1:length(stgList)) {
    stg <- stgList[k] 
    cdfLine <- cdfLines[k]
    ...
    plot(x0$x,x0$cd, type = "l",lty = cdfLine, col=incCol[k], xlab = "", ylab = "",
         xaxs = "i",xlim = c(0, cdfUp), axes = F, yaxs = "i", ylim = c(0, 1),
         lwd = 2.5,main = paste0("ECDF of minimum adult population at ", site," removing ",stg,"s"))
    axis(1,at = seq(tickLow, tickUp, by = tickInc), cex.axis = 1.5, lwd=2)
    axis(2,las = 1,cex.axis = 1.5,lwd = 2)
    #minor.tick(nx = 2, ny = 0, tick.ratio = 1)
    lines(x25$x,x25$cd, type = "l", lty = cdfLine, col = incCol[k], lwd = 2.5)
    lines(x50$x,x50$cd, type = "l", lty = cdfLine, pch = "", col=incCol[k] ,lwd = 2.5)
    lines(x75$x,x75$cd, type = "l", lty = cdfLine, pch = "", col=incCol[k] ,lwd = 2.5)
...
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
# Figure 6 is generated in the global sensitivity analysis code block
